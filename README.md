...
| 3 | **음성 AI 비서** | 지도를 볼 수 없는 이동 중 상황에서 "다음", "양식 먹고 싶어" 같은 자유발화로 추천 탐색 (Upstage Solar 의도분류 + 키워드 분류기 폴백). |
...
2. **대안 장소 추천** — 혼잡 임계 초과 시 SPOT 랭킹 + Upstage Solar 가 다듬은 추천 사유 카드 + 카카오맵 길안내 연결
...
| AI | SPOT 추천 엔진(자체) · Upstage Solar `solar-pro3`(추천 사유·음성 의도 — 키 없거나 실패 시 결정적 폴백) · 선호 벡터 피드백 학습 |
