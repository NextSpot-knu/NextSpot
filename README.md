...
포화한 인기 관광지 대신, 사용자의 취향·예상 대기·도보 거리를 종합한 **SPOT(Smart Place Optimization for Tourism)** 점수로 **반경 150m 내 한산한 대안 장소**를 추천하여 도심 전체의 관광 수요를 고르게 분산합니다.
...
- ✅ **그대로 재사용:** 모노레포 아키텍처(Next.js 웹 + FastAPI + Supabase + 로컬 sklearn)와 **SPOT 추천 엔진**.
...
```
SPOT_Score = w₁ · 취향 일치율 − w₂ · (예측 대기시간 + 이동시간) + w₃ · 인센티브
...
2. **대안 장소 추천 엔진 (Alternative Suggestion Engine)** — 목적지 혼잡 임계 초과 시 반경 150m TourAPI POI를 SPOT로 정렬. '취향 일치율 00% · 예상 대기 0분 · 도보 0분' 형태로 추천 사유 투명 제공.
...
│   └── api/            # FastAPI — SPOT 추천 / 혼잡 예측 백엔드
│       └── app/services/spot/   # ★ 재사용 핵심: score·preference·travel·wait_time
...
| 단계 | 기간 | 목표 |
| --- | --- | --- |
| MVP | 2026.05 ~ 2026.09 | 경주 황리단길 웹앱, TourAPI 연동, SPOT 추천 엔진, 혼잡 지도, 앱 래핑 |
