...
export default function RecommendationComparison({ recommendations }: { recommendations: RecommendationResponse[] }) {
  ...
  const comparisons = buildSpotComparisons(top.map((item, index) => ({
  ...
  const comparisonCards = (
    <div className="space-y-2">
      {top.map((item, index) => {
        const comparison = comparisonById.get(item.recommendationId);
  ...
  if (!open) return (
    <section className="mb-4 rounded-2xl border border-jade/25 bg-jade/5 p-4">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h3 className="text-sm font-extrabold text-muk">{t('recommend.spotComparison.title')}</h3>
      </div>
      {comparisonCards}
      <p className="mt-2 text-[10px] leading-relaxed text-muk-soft">{t('recommend.spotComparison.honesty')}</p>
  ...
  return (
    <section className="mb-4 rounded-2xl border border-line bg-white p-4 overflow-hidden">
      ...
      <div className="mb-4 rounded-xl bg-jade/5 p-3">
        <p className="mb-2 text-xs font-extrabold text-muk">{t('recommend.spotComparison.title')}</p>
        {comparisonCards}
        <p className="mt-2 text-[10px] leading-relaxed text-muk-soft">{t('recommend.spotComparison.honesty')}</p>
      </div>
