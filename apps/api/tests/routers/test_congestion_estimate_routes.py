...
계약(추가만 — 구 번들은 모르는 필드라 지나간다):
  ...
  · /recommendations(/by-type): congestion_source 가 'none' 일 때만 congestion_estimate.
                              congestion_source 는 'none' 그대로 — 'estimated' 는 새 값으로 나가지 않는다
...
def test_by_type_shows_the_estimate_and_uses_it_only_as_the_crowded_gate(auth_client, estimates_for):  # noqa: F811
    ...
    # 실측(단건 제보라도)이 있는 곳은 화면에서 실측이 이긴다 — 추정은 붙지 않고, 순위에도 안 쓰인다.
    assert after["c-3"]["congestion_source"] == "measured"
    assert after["c-3"]["congestion_estimate"] is None
    assert after["c-3"]["breakdown"]["ranking_congestion"] is None
