...
@pytest.fixture(autouse=True)
def _isolate_admin_coalesce():
    """관리자 집계 합류(app/core/admin_coalesce.py)의 세대·진행 중 표를 테스트마다 비운다.
    합류는 결과를 저장하지 않으므로 순차 호출 테스트에는 영향이 없다 — 이건 테스트 사이 누수 방지용."""
    from app.core import admin_coalesce

    admin_coalesce.reset_for_tests()
    yield
    admin_coalesce.reset_for_tests()
