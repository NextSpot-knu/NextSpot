...
class Settings(BaseSettings):
    ...
    # 5곳 = 10분 주기로 하루 720회. 둘로 나뉜다(2026-09-20 실측 근거):
    #  · 시연 권역(홍대·연남동·합정역) — 걸어서 오갈 수 있어 "붐비는 곳 대신 옆 동네" 를 실시간 인구로 보여 준다.
    #  · 검증 지점(명동·동대문) — 서울에서 **실시간 주차 대수를 주는 거의 유일한** 곳이라(홍대·연남·합정은 0곳)
    #    경주와 같은 조건(주차만 있는 상태)의 추정치를 실측 인구와 대조할 수 있다.
    # 인증키 일 호출 한도가 720보다 낮으면 앞쪽 3곳만 남긴다.
    SEOUL_CITYDATA_TARGETS: str = (
        "홍대 관광특구,연남동,합정역,명동 관광특구,동대문 관광특구"
    )
