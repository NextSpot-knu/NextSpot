...
- Python **3.11**, Node **20+** (PATH 등록 — `package.json` engines 기준)
  > 백엔드 기준 버전은 3.11 입니다(CI·`apps/api/Dockerfile`와 동일). httpx 가 **3.14+ 와 호환되지 않아**
  > PATH 의 `python` 이 3.14 면 import 단계에서 깨집니다. 직접 명령을 칠 땐 `py -3.11` 을 쓰세요.
...
> `PYTHONUTF8=1` 을 설정합니다. 3.14 등 비호환 버전이 잡히면 새 창에서 조용히 죽는 대신
