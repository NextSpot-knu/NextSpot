...
- Python **3.11**, Node **20+** (PATH 등록 — `package.json` engines 기준)
  > 백엔드 기준 버전은 3.11 입니다(CI·`apps/api/Dockerfile`와 동일). 이 머신의 Python 3.14 환경은
  > `websockets` 12.0에 `websockets.asyncio`가 없어 앱 import가 깨집니다. 직접 명령은 `py -3.11`을 쓰세요.
...
```
> 스크립트가 인터프리터를 `apps/api/.venv` → `py -3.11` → PATH `python` 순으로 해석하고
> `PYTHONUTF8=1` 을 설정합니다. `websockets.asyncio`가 없는 이 머신의 3.14가 잡히면 새 창에서 조용히 죽는 대신
> 즉시 안내와 함께 중단합니다. venv 를 만들어두면(`py -3.11 -m venv apps\api\.venv`) 그걸 우선 사용합니다.
