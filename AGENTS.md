...
- 저장소는 `C:/Users/hennr/Desktop/NEXTSPOT` (2026-07-18 이전의 중첩 폴더
  `Desktop/nextspot/NextSpot`는 더 이상 없음). Windows 11 + PowerShell.
  ⚠️ 이동 과정에서 `apps/api/.env`가 소실됨 — 복원 전까지 로컬 백엔드 구동 불가(HANDOVER §-21).
- 백엔드 Python은 **3.11**(CI·Dockerfile와 동일). PATH의 `python`은 3.14이고 이 환경의 낡은
  `websockets` 12.0에는 realtime 2.31.0이 요구하는 `websockets.asyncio`가 없어 import에서 깨진다
  — 직접 명령을 칠 땐 항상 `py -3.11` + `PYTHONUTF8=1`.
- 로컬 구동: `.\run_local.ps1` (백엔드 8000 + 프런트 3000). 스크립트가 인터프리터를
  venv → `py -3.11` → PATH `python` 순으로 해석하고 `PYTHONUTF8=1`을 설정한다.
  **3.14+는 거부**, 3.12/3.13은 CI 불일치 경고 후 진행, 3.11이면 조용히 통과.
  venv를 만들어두면(`py -3.11 -m venv apps/api/.venv`) 그걸 우선 사용한다.
- 백엔드만 수동으로 띄울 때:
  ```powershell
