...
`gemini` CLI(0.50.0)는 설치돼 있으나 **인증 필요** — `GEMINI_API_KEY` 환경변수를 넣거나
`gemini` 를 인자 없이 실행해 Google 로그인(OAuth)을 1회 마쳐야 한다(대화형이라 사람이 해야 함).
인증 전에는 `-p` 프롬프트가 auth 안내만 출력하고 아무 일도 하지 않는다.
...
Claude 세션의 병목은 컨텍스트와 사용량 한도다. Codex(ChatGPT 구독)는 **별도 쿼터**를 쓰므로,
일을 넘길수록 Claude의 한도가 남는다. Gemini(무료 쿼터)도 같은 역할을 할 수 있으나
**§2의 인증을 마쳐야 쓸 수 있다** — 그전까지 오프로드 대상은 Codex 하나다.
