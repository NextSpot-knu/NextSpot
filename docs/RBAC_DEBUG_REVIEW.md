...
# 3부. 분산코스 간헐 실패 — 진행 중 (가장 중요)

## 상태

**대표 기능이 프로덕션에서 아직 완전히 안 낫습니다.** 개선은 했지만 남아 있습니다.

| 시점 | 후보가 있는 요청의 결과 |
|---|---|
| 발견 당시 | 500 — 거의 항상 실패 |
| 503 전환 + 부분 실패 허용 후 | 5/8 성공 |
| 연결 재시도 1→3회 후 | 50% |
| 재시도 시 풀 정리 추가 후 | **69%** (16회 중 11회) |

후보가 없는 요청(새벽이라 여는 곳이 없는 경우)은 200 + 빈 배열로 정상 동작합니다.

## 원인 (로컬 재현으로 확인)

프로덕션과 같은 조건(신규 익명 사용자, 목킹 없음)으로 로컬에서 재현해 스택을 잡았다:

```
httpx.RemoteProtocolError: <ConnectionTerminated error_code:1, last_stream_id:53>
  at fetch_user → supabase_client.table("users").select("*")
```

Supabase 가 HTTP/2 연결을 GOAWAY 로 닫은 뒤, 풀에 남은 그 연결을 다시 집으면 나는 오류다.
서버 장애가 아니라 **연결 재사용 문제**이고, 그래서 "같은 요청이 한 번은 실패 다시 하면 성공"
이라는 관찰과 맞는다.

실패가 2~3회씩 뭉쳐 나온 것도 근거다 — 무작위가 아니라 풀 안의 죽은 연결을 계속 집는 모양이다.

## 고친 것

1. `/courses/recommend` 에 예외 처리 추가 — 500 → 503(구분 가능한 안내), 스택 로깅
2. 후보 평가를 `return_exceptions=True` 로 — 후보 하나가 터져도 나머지로 코스를 만든다
   (전부 실패하면 올린다 — 빈 코스로 위장하지 않는다)
3. stale 연결 재시도 1회 → 3회(짧은 백오프)
4. 재시도 전 풀을 닫아 새 연결 강제

## 남은 것 — 여기서 막혔습니다

로컬에서는 **재현이 안 됩니다.** 같은 동시성 패턴(12스레드 × 6라운드, idle 섞음)으로
HTTP/2·HTTP/1.1 둘 다 72/72 성공. Render 네트워크 경로에서만 나는 현상으로 보입니다.

**다음 한 걸음(제가 못 하는 것)**: Render 로그에서 `course_recommend_failed` 를 찾아
`exception` 필드의 스택을 보면 남은 31% 의 정체가 바로 나옵니다. 제가 exc_info 를 넣어
뒀으므로 전체 스택이 찍혀 있습니다. 그 한 줄만 주시면 이어서 고치겠습니다.

만약 여전히 `RemoteProtocolError` 라면 다음 후보는:
- Supabase 클라이언트를 HTTP/1.1 로 (PostgREST 에 멀티플렉싱이 필요 없다)
- 요청마다 새 연결(keepalive 비활성) — 느리지만 확실하다
- Render 유료 플랜(free 는 네트워크·메모리 제약이 있다)

## 시연 관점

새벽이라 여는 곳이 없어 지금은 빈 코스가 정상입니다. **낮 시간대에 다시 확인해 주세요** —
실제로 코스 3개가 나오는 것은 확인했습니다(위 2차 측정 2회차·4회차).
심사 전에는 이 항목이 가장 먼저 해결돼야 합니다.
