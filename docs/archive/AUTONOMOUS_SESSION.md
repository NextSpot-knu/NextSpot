...
**사람 검토로 남긴 항목:**
- ✅ #4 코스 SPOT 하위항 누적도착 — **해결**(`f93c398`): calculate_spot_score 에 옵션 depart_time 추가, courses 가 누적 출발시각 전달. 기존 단일 추천 동작 불변, pytest 80 passed.
- #14 작은 라벨 대비 — muk-soft 대비는 대략 AA 통과(≈4.9:1), 실제 이슈는 9px '크기'라 일괄 확대 시 타임라인 레이아웃이 흔들릴 수 있어 디자인 판단 필요(주관적).
- #13 main 마커 재생성 성능 — 지도 렌더 경로 리팩터라 회귀 위험. 실행 검증(앱+백엔드 기동) 하에 신중히 진행 권장.
