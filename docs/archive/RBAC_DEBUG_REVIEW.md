...
# 3부. 분산코스 간헐 실패 — **해결됨** (2026-08-28 오후)

## 결론 먼저

**해결됐습니다.** 사람 사용 간격(2초)으로 30회 연속 200, 전부 정류지 3개 정상 생성,
평균 응답 3.66초. 아래는 어떻게 좁혀 갔는지의 기록입니다.

원인은 하나가 아니라 **셋이 겹쳐 있었습니다**:

1. 예외 처리 부재 — 업스트림이 흔들리면 그대로 500 (`/courses/recommend` 에 try/except 가 없었다)
2. 재시도가 닿지 않는 위치 — stale 연결 오류는 `handle_request` 가 아니라 **본문을 읽는 중** 난다
3. **요청당 작업량** — 코스 1회에 Supabase 요청 15건, 그중 11건이 쓰지도 않는 영업 근거

셋째가 결정타였습니다. 15건 → 5건으로 줄이자 성공률이 56% → 100% 가 됐습니다.
...
`fetch_all_facilities()` 가 시설 전체분의 영업 근거를 받아 왔다. PostgREST 의 in.(...) URL
한계로 150개씩 끊으므로 1,600곳이면 **요청 11건**이다. 그런데 `courses.py` 에는
availability 참조가 한 줄도 없다 — 쓰는 곳은 후보 12~24곳을 평가하는
`open_status_at_arrival` 하나뿐이고, 나머지 1,580곳분은 받아서 버렸다.

(그 함수 주석에 "시설은 85곳뿐이라" 가 남아 있었다. 그때는 싼 호출이었다.)

후보 풀이 확정된 뒤 그 몇 곳만 조회하도록 옮겼다:

```
콜드 25건 → 15건 · 웜 15건 → 5건 · availability 11건 → 1건
코스 결과 동일(정류지 3개) · 신선도는 오히려 나아짐(평가 직전 값)
```

## 남은 것 — by-type 동시 버스트

분산코스는 실사용 조건에서 100% 지만, **동시에 8개를 몰아치면** 여전히 절반쯤 503 이 난다.
실제 페이지 로드 패턴(서로 다른 엔드포인트 8개 동시 + 이어서 코스 1회)으로 재보면:

```
페이지 로드 실패 0~3/8 (전부 by-type) · 이어지는 분산코스 3/3 성공
```

즉 남은 것은 **프런트가 by-type 을 한꺼번에 3개 부르는 구간**이다. 프런트는 이미 503 을
잡아 로컬 폴백으로 내려가므로(콘솔: "by-type 추천 실패 → 영업 확인 후보만 로컬 폴백")
사용자에게는 추천이 계속 보인다 — 다만 서버 근거 대신 로컬 규칙이 쓰인다.

개선하려면(검토 필요):
- 프런트가 by-type 3개를 순차로 부르거나, 백엔드에 묶음 엔드포인트를 둔다
- Render 유료 플랜 — free 는 CPU 1개·워커 1개라 동시 버스트에 약하다

우선순위는 낮다. 대표 기능은 정상이고 이 경로는 이미 우아하게 degrade 한다.

## 추가 조사 (2026-08-28 오전)

### 새로 안 것 — 코스만의 문제가 아니다

브라우저 네트워크 탭에서 메인 화면 한 번 로드에 **API 요청이 동시에 10개** 나가고,
그중 `/recommendations/by-type` 도 **503** 을 내는 것을 확인했다. 같은 실패 클래스가
앱 전반에 걸쳐 있다는 뜻이다.

동시 요청이 방아쇠라는 점도 이걸로 분명해졌다 — 로컬 재현이 안 됐던 이유이기도 하다
(내 PC 는 Supabase 까지 지연이 짧아 경합 구간이 좁고, Render 는 훨씬 넓다).

### 시도했다가 되돌린 것 — HTTP/1.1

httpx 동기 클라이언트에서 `http2=True` 는 동시 요청을 TCP 연결 하나에 다중화한다.
여러 스레드가 같은 클라이언트를 두드리므로 H2 상태가 깨지면 연결 전체가 죽는다 —
어젯밤 잡은 `ConnectionTerminated` 와 맞는 가설이었다.

`http2=False` 로 바꿔 배포했더니 **성공률이 40% → 0% 로 떨어졌다.** 12회, 8회 두 번 측정 모두
전부 503. 즉시 되돌렸고(`dc8f9ae`) 되돌린 뒤 50% 로 복귀한 것까지 확인했다.
같은 가설로 다시 바꾸지 않도록 코드에 실측 수치와 함께 주석을 남겼다.

### 다음으로 의심되는 것 — 서버 동시성

Render free 플랜 + uvicorn **워커 1개**에 페이지 로드마다 동시 요청 10개가 몰린다.
`/infrastructures` 는 단독으로도 2.3~3.2초가 걸리므로, 큐잉만으로 4초를 넘길 수 있다
(타임아웃을 4초로 올렸는데도 브라우저에서는 여전히 폴백으로 넘어간다).

확인해 볼 것:
- 프런트가 첫 화면에서 API 를 10개 동시에 부를 필요가 있는지 (직렬화·지연 로드 여지)
- uvicorn 워커 수 — 늘리면 프로필 캐시가 워커별로 갈라지는 점을 함께 고려해야 한다(§3-4)
- Render 유료 플랜

**여전히 Render 로그의 `course_recommend_failed` 스택이 가장 빠른 길입니다.**
