# HANDOVER 로그 (2026-06-30 ~ 2026-08-28) — 읽기 전용 아카이브

> 2026-09-04 이전의 `docs/HANDOVER.md` 전문을 그대로 옮겼다. 현재 상태는 [`../HANDOVER.md`](../HANDOVER.md)가 정본이다.
> 절 번호는 당시 체계 그대로다 — 음수가 최신(-45)이고 아래로 갈수록 오래됐으며, 맨 아래 0~6절이 최초 인계 문서다.
> ⚠️ -20·-27·-28·-29 번호는 각각 두 번 쓰였다(당시 실수). 다른 문서의 "§-28" 인용은 두 곳 중 문맥으로 고른다.
> 이 파일에는 더 이상 항목을 추가하지 않는다 — 새 세션 기록은 `docs/HANDOVER.md` "최근 세션"에 쓰고, 넘치는 항목만 이 파일 맨 위(이 안내 아래)로 옮긴다.
...
Suspense 경계 없이 썼다. `output:'export'` 빌드는 그 조합에서 해당 라우트 프리렌더에 실패하고,
`next build` 가 통째로 죽는다. CI 는 build 를 돌리므로 푸시했으면 잡혔다 — 로컬에서 라우트를
고치고 빌드 없이 커밋한 게 원인이다. **프런트 라우트를 건드리면 커밋 전에 빌드 한 번.**

**스케줄러 수집이 배포되는 순간 멈출 뻔했다.** `/area-demand/snapshots/collect` 의 가드를
`require_admin` → `require_role(ROLE_ADMIN)` 로 바꿨는데, 이 경로의 주 호출자는 사람이 아니라
Supabase pg_cron(10분 주기)과 GitHub Actions 다. 둘 다 세션을 가질 수 없어 401 을 받는다.
응답을 보는 사람이 없어 알림도 안 뜨고 시계열만 조용히 멈춘다 — availability 청킹과 같은
실패 모양이다. 배포 전에 막아 실제 중단 구간은 없었다(`origin/main` 이 아직 구 가드였고,
스냅샷이 16:03~16:43Z 10분 간격으로 계속 쌓이고 있었다).

`require_machine_or_role()` 을 추가했다. 폐지한 공유 토큰과 다른 점이 핵심이다: 토큰이
프런트로 나가지 않고(`NEXT_PUBLIC_*` 금지), 관리자 API 전체가 아니라 **수집 트리거 한 경로**에만
유효하며, `hmac.compare_digest` 로 비교한다. `SERVICE_API_TOKEN` 이 있으면 그것만, 없으면
`ADMIN_API_TOKEN` 으로 폴백 — 이미 적용된 pg_cron 함수와 Actions Secret 을 동시에 바꾸지 않고
회전할 수 있게 하기 위해서다. 헤더도 `X-Service-Token` 을 정식으로 하되 구 헤더를 계속 받는다.

**심사 승인/반려가 증빙을 상태 갱신보다 먼저 지웠다.** 갱신이 실패하면 요청은 `pending` 인 채
증빙만 사라져 다시 심사할 수도, 신청자에게 돌려줄 수도 없다. 순서를 뒤집었고, 테스트가
"삭제 시점의 status" 를 직접 관찰해 순서를 잠근다.

**테스트를 붙였다.** `/dev` 콘솔(45개) — 앱에서 가장 강한 권한 표면인데 12개 라우트 전부
무방비였다. 기존 `test_routers.FakeSupabase` 는 체이닝을 흡수해 정해둔 데이터만 돌려주고
필터링도 변경도 하지 않아 상태 전이 검증에 쓸 수 없다 — `_MiniSupabase` 를 따로 뒀다.
프런트는 `canEnter*` 를 `lib/accountRoles.ts` 로 빼서 React 없이 테스트 가능하게 했다.

**실측 검증**: 실제 Supabase 의 `users.role` 로 4역할 × 5경로 매트릭스(merchant 는 자기 가게만
200/남의 가게 403, admin 은 사장님 콘솔 403, 폐지 헤더 전 경로 401), 로그인한 관광객 토큰으로
권한 상승 6종 시도 전부 차단(가드 트리거 400 + RLS 403 + 0행 영향).

⚠️ **그 매트릭스를 실 DB 로 돌리다 프로덕션에 가짜 `verified` 행 3개를 썼다.** 즉시 정리했고
잔여 0건을 확인했지만, 스테이징이 없다는 사실 자체가 문제다. **읽기만 실 DB, 쓰기는 대역으로.**

미결(판단 필요): 집중률 상대지수를 절대 점유율처럼 0.7/0.3 블렌딩하는 문제,
developer 방송이 `verified` 로 학습에 들어가는 문제, `ADMIN_API_TOKEN` 회전(구 값은
`NEXT_PUBLIC_` 으로 번들에 미러됐던 값이라 공개된 것으로 취급), `/voice/turn` 유량 제한.
...
- 운영에서 "계정 생성이 안 된다" 증상을 조사했다. Supabase 가입 설정·신규 이메일 가입·익명 승격
  모두 정상이었고, 실제 원인은 **이미 가입된 이메일로 재가입 시도** 시 서버가 `email_exists`(422)를
  반환하는데 화면이 모든 실패를 "다시 시도해 주세요" 하나로 뭉개 원인을 숨기던 것.
- `lib/authErrors.ts`의 `classifySignUpError`가 GoTrue `error_code`(email_exists/
  user_already_exists/weak_password)와 메시지 폴백으로 원인을 분류한다. 이미 가입된 이메일은
  "로그인 탭에서 로그인해 주세요" 안내와 함께 로그인 탭으로 자동 전환하고, 6자 미만 비밀번호
  (서버 실측 최소 6자)는 전용 문구를 보여준다. 그 외 실패는 기존 일반 안내를 유지한다.
  ko/en/ja/zh 4로케일 동시 추가(parity 723 keys).
- 검증: 단위 테스트 `lib/authErrors.test.ts`(테스트 체인 등록), lint 0 errors(기존 warnings),
...
- 관광객 화면은 경주 현지 시각(KST) `18:00~06:00`에 자동으로 한옥 먹빛 다크모드를 적용한다.
  기기 시간대가 다른 외국인 관광객도 경주 시각을 기준으로 판정하며, 첫 렌더 전 부트스트랩 스크립트로
  적용해 밝은 화면이 잠깐 번쩍이는 현상을 막았다.
- 설정 화면에 `자동 / 라이트 / 다크` 선택을 추가했고 `nextspot_theme`에 저장해 재방문 때 복원한다.
  한국어·영어·일본어·중국어 문구를 모두 제공하며 관리자·사장님 화면은 기존 전용 테마를 유지한다.
- 한지 배경·본문·보조문구·테두리·금빛 강조를 런타임 CSS 토큰으로 전환했고, 흰 카드·토스트·하단
  내비게이션·지도 타일까지 야간 대비를 맞췄다. Kakao 지도는 공식 다크 스킨이 없어 관광객 지도
  타일에만 저휘도 필터를 적용하고 NextSpot SVG 마커는 제외한다.
- 검증: 테마 경계/경로/4개 언어 단위 테스트, typecheck, lint 0 errors(기존 warnings), 정적 build
  34 pages, Playwright 모바일·추천·방문·음성 전체 **17/17**, `git diff --check` 통과. 외부 Kakao SDK는
  E2E에서 빈 응답으로 고정해 네트워크 상태가 테스트 결과를 바꾸지 않게 했다.

## -39. 2026-08-25 — 유명 명소를 기준으로 한 ‘같은 경험’ 대안 추천

- 메인 검색 아래 `경주가 처음이라면` 진입점을 추가했다. 신라 핵심 산책·야경·황리단길 한옥 카페·
  실내 역사·교촌/월정교 산책을 고르면 유명 장소를 목적지로 강제하지 않고 추천의 기준점으로 쓴다.
- 테마 요청은 기존 `/recommendations`의 원본 장소 재배치 경로를 사용한다. 후보 유형·테마 ID를 함께
  보내고, TourAPI 연관 목적지이거나 실제 장소명·분류·소개에 테마 근거가 있는 후보만 영업 가능성·
  실제 보행 경로를 거쳐 기존 SPOT 점수순으로 정렬한다. 근거가 없으면 임의 후보로 3개를 채우지 않는다.
- 화면에는 기준 명소, 테마 근거, 상위 3개 대안의 이름·도보시간·1위 대비 실제 SPOT 입력 차이를
  함께 보여준다. 테마 선택은 SPOT 가중치 `0.4/0.4/0.2`를 바꾸지 않고 후보 자격만 좁힌다.
- 테마 ID와 기준 시설은 추천 스냅샷에 저장되어 길찾기·방문 결과와 조인할 수 있다. 익명 세션 생성 전
  고정 모의 사용자로 추천하던 레거시 `/recommendations` 클라이언트 경로도 공용 세션 Promise로 교체했다.
- 한국어·영어·일본어·중국어 문구와 키/placeholder 패리티를 추가했다. 검증: API pytest **804 passed**,
  API ruff, 웹 typecheck/unit, lint 0 errors(기존 warnings), 정적 build 34 pages, Playwright **16/16**,
  `git diff --check` 통과.
...
- 영업시간이 없는 카페·식당은 기존처럼 Kakao Local의 공식 장소 상세로 먼저 보낸 뒤, 돌아온 카드에서
  `영업 중 / 닫힘 / 잘 모르겠어요`만 묻는다. 위치·이동 경로·자유입력은 수집하지 않는다.
- 인증 API `POST /api/v1/reports/availability`는 서버 시각으로 사용자·장소별 최신 상태 하나만 저장한다.
  서로 다른 사용자의 최근 일치 제보가 2건 이상이고 반대 상태보다 많을 때만 `corroborated`가 된다.
  동률·단일 제보는 추천 자격을 바꾸지 않는다.
- 확인 근거는 영업 중 30분, 닫힘 60분 뒤 만료된다. 유효한 `corroborated`만 도착시점 영업 판정에
  반영하며, 추천 스냅샷에도 사용한 근거를 보존한다. DB/RPC migration은
  `20260825190000_add_facility_availability_reports.sql`이다.
- 추천 카드에는 상태·확인 인원·신선도를 4개 언어로 표시한다. 닫힘 확인은 해당 후보를 넘기고,
  제보 저장 실패 시 추천 자체를 망가뜨리지 않고 재시도 문구를 보여준다.
- 운영 반영 순서: migration 적용 → API 배포 → web 배포 → 서로 다른 테스트 계정 2개로 같은 장소를
  같은 상태로 확인 → 세 번째 추천에서 corroborated 근거 및 영업 판정 반영 확인.

## -36. 2026-08-25 — 관광객 계정 DB·복구·탈퇴 완성

- 익명 세션 Promise를 생성 중에만 공유하도록 바꿨다. 이메일 로그인이나 로그아웃 뒤 과거 게스트 uid가
  현재 JWT와 섞여 추천 API가 403이 되던 계정 전환 결함을 제거했고, 인증 이벤트마다 저장·방문 큐를
  현재 uid로 다시 동기화한다.
- 기존 계정 로그인 시 게스트 데이터를 여러 REST 요청으로 옮기던 코드를 service-role 전용
  `merge_guest_account_data` RPC 하나로 교체했다. 추천·피드백·방문 결과·저장 장소·쿠폰·취향 벡터·
  혼잡 제보·문의·프로필 집계를 한 DB 트랜잭션에서 이동하며, 중복 장소는 기존 계정 행을 보존하고
  재시도는 멱등이다.
- 마이페이지 표시 이름은 `public.users.nickname`에 저장되어 다른 로그인 기기에서 복원된다. 이메일
  계정 배지도 실제 이메일로 표시한다.
- `/forgot-password`와 `/auth/reset-password`를 추가해 Supabase 복구 메일로 비밀번호를 다시 설정한다.
  설정 화면의 계정 삭제는 인증된 현재 사용자만 FastAPI를 거쳐 `auth.users`에서 삭제하고, FK cascade로
  프로필·저장·추천·방문·쿠폰·문의 데이터를 함께 제거한다. 혼잡 제보 원본은 사용자 연결만 제거되어
  익명 통계로 남는다.
- 복구 메일은 이미 허용된 `/auth/callback`을 공유하므로 새 Redirect URL 설정은 필요 없다. 운영
  Supabase에는 migration `20260825120000_atomic_account_merge.sql`을 적용했고, service-role 동일 uid
  무변경 호출이 모든 카운트 0으로 성공해 RPC 등록·권한을 확인했다. 남은 운영 반영은 web/api 배포다.
...
- 운영 Supabase의 실제 테이블·핵심 컬럼을 확인한 뒤 누락된 migration history 8건을 `applied`로
  ...
  수집 결과 `observed_at=2026-08-24T10:21:21Z`, `bucket_at=10:20:00Z`, DB
  `bucket_minutes=10`, 주차장 3곳·총 880면·가용 174면으로 확인했다.
- 한국관광공사 집중률은 주차 점유와 하나의 절대 혼잡도로 표시하지 않는다. 추천 응답과 3개 사용자
  화면에서 기준 관광지, 후보와 거리, 예측 날짜, 원본 상대지수를 독립 근거로 표시하고, 해당 관광지
  자체 최고 시기=100인 일별 상대지수이며 매장 내부 혼잡·대기시간이 아님을 한국어·영어·일본어·
  중국어로 설명한다.
- 이름만 있는 관광 통계를 좌표 앵커로 삼기 때문에 NFKC·공백·안전한 메타데이터 차이만 정규화하고,
  관광지/문화시설의 유일 일치만 허용한다. 중복 이름, 카페·식당, 지점 구분 괄호는 fail-closed다.
- 2026-08-24 운영 읽기 전용 평가: 직접 앵커 10곳(기존 exact와 동일), 2km 전파 1,037/1,624곳
  (63.85%), 12개 통제 시나리오의 Top 3 근거 보유 100%, 충분히 구분되는 수요 4/12(33.33%),
  Top 3 동일 앵커 쏠림 4/12(33.33%), 통제 순위 변화 2/12(16.67%)다. 이는 실제 개인화 정확도나
  장소 내부 혼잡 검증이 아니라 관광 통계의 구분력 평가다.
- 검증: API pytest **773 passed** 및 ruff, 웹 typecheck/unit(4개 언어 695키), lint 0 errors
  (기존 warnings 191), 정적 build 32 pages, `git diff --check` 통과.

## -33. 2026-08-21 — 2학기 전 자체 해결 품질 패스

- 경북대·관광기관·B2G 접촉은 2학기 개강 뒤 검토하기로 했다. 그 전에는 외부 협조 없이 해결 가능한
  장소 정확도, 영업 신뢰, 보행시간, 첫 추천 속도와 배포 완성도에만 집중한다.
- 월정교의 수동 검증 Kakao 좌표·주소·영업시간·장소 ID를 반영하는
  `20260821150000_verify_woljeonggyo.sql`과 회귀 테스트를 추가했다. 운영 Supabase에는 아직 적용하지
  않았으므로 마이그레이션 적용 전 배포 화면의 월정교는 이전 상태일 수 있다.
- 정적 export의 Open Graph/Twitter 이미지가 `localhost`로 생성되던 경고를 없앴다.
  `NEXT_PUBLIC_SITE_URL`을 추가하고 기본 운영 주소를 `https://nextspot-nu.vercel.app`로 두어
  `/og.png` 절대 URL을 생성한다.
- 사용자 홈의 우연한 lockfile을 Turbopack 루트로 오인하던 빌드 경고를 제거하고 저장소 루트를
  명시했다. 동일 환경의 컴파일은 17.6초에서 9.8초로 줄었지만 캐시 영향이 포함될 수 있으므로
  운영 사용자 응답속도 개선 수치로 해석하지 않는다.
- 검증: API pytest **758 passed**, ruff clean, 웹 typecheck/unit(4개 언어 695키), lint 0 errors
  (기존 warnings 190), 정적 build 32 pages, journey-loop Playwright **8/8**, `git diff --check` 통과.
  이번 변경은 아직 커밋·푸시·원격 마이그레이션·배포하지 않았다.
...
- NextSpot의 현재 상태는 `개인용`이 아니라 **무광고·무과금·비수익 공익형 공개 실증/공모전 서비스**다.
  공익 목적이 공급자 약관의 비상업 이용을 자동 보장하지 않으므로, 공개 배포·상금 가능성·향후 사업화
  계획을 밝히고 제한 실증 허가와 화면 표시·파생 등급 권리를 서면으로 받는다.
- 제휴는 관광공사 15128555 개발승인 완료(2026-08-24) → SKT 경주 커버리지 시험 → 공모전 멘토를 통한 경북관광기업
  지원센터/경주스마트미디어센터 소개 → 로플랫·ENCLE 제한 실증 순으로 진행한다. 사용자의 명시적 승인
  없이 에이전트가 업체나 기관에 실제 문의를 보내지 않는다.

## -31. 2026-08-21 — 장소·영업·보행 신뢰 계약과 검색 범위 확장

- Kakao Local의 장소 ID·주소·좌표를 한 레코드로 보존해 경주 카페·식당 검색 모집단을 확장했다.
  외부 ID가 없는 과거 데모 시설 15개는 삭제하지 않고 비활성화했다. 운영 검색 모집단과 추천
  후보를 분리하며, 검색 결과가 곧 추천 자격을 의미하지 않는다.
- 도보 시간은 번들된 OpenStreetMap 보행 그래프의 최단경로로 계산한다. 이치니산도→피자옥은
  직선 138m가 아니라 보행 약 196m·2.9분으로 회귀 테스트하며, 화면은 소수점을 올림 표시한다.
- 도착시점에 닫힘이 확인된 음식점·카페는 추천하지 않는다. 영업시간을 모르는 장소는 영업 중으로
  만들지 않고 확인 필요 경고를 표시한다. 운영 카페·식당 1,567곳 중 1,513곳(96.55%)에 영업시간이
  없어 여전히 가장 큰 데이터 병목이다. 다만 황리단길 중심 10분 보행권의 확인된 후보는 평일
  11시 20곳, 14시 22곳, 19시 18곳이며 새벽 2시는 0곳으로 정확히 비운다.
- 주차장 탭은 경주시 ITS 공영주차의 실제 잔여면과 관측 시각을 별도로 보여준다. 주변 수요 카드에는
  계산에 사용한 반경을 표시해 매장 내부 혼잡과 구분한다.
- 장소 검색은 상호·주소뿐 아니라 검증된 대표메뉴/취급메뉴, Kakao 업종, 음식 태그, 소개를 찾는다.
  `돼지고기→삼겹살·돼지갈비`, `구이→고깃집·숯불구이`처럼 제한된 동의어를 Kakao에 재조회하지만
  그 동의어를 해당 가게의 대표메뉴로 저장하거나 표시하지 않는다.
...
- 장소 내부 혼잡·검증 대기가 없을 때 추천 카드가 이동시간만 보여주던 문제를 수정했다. 경주 ITS
  공영주차와 관광 통계에서 계산한 `areaDemandLevel`이 있으면 카드 첫 상태를 `주변 수요: 한산/보통/붐빔`으로
  표시하고 `실시간 주변 수요` 또는 `통계 기반 주변 수요` 출처를 함께 보여준다. 매장 내부 좌석이나 대기로
  오인되지 않도록 장소 혼잡과 주변 권역 수요 라벨은 계속 분리한다.
- 메인 추천 사유와 대기 없는 여행 보드도 같은 주변 수요 근거를 사용한다. 검증된 예상 대기시간이 없으면
  가짜 `0분` 대신 주변 수요를 우선 표시하고, 그 근거도 없을 때만 `대기 정보 수집 중`으로 폴백한다.
- TourAPI 장소 소개는 3줄에서 6줄까지 보여주도록 늘렸다. 원문이 짧은 장소는 내용을 생성하지 않고 실제
  overview 길이만큼만 표시한다.
- SPOT 가중치 `0.4/0.4/0.2`, 시설 내부 혼잡·대기 생성 금지, 30분 실시간 신선도 제한은 변경하지 않았다.
- 검증: web typecheck, unit(음성 29/29·4개 언어 658키 패리티), journey-loop Playwright 8/8,
  webpack 정적 build 32 pages와 `git diff --check` 통과.

## -29. 2026-08-20 — 첫 추천 신뢰성·체감 속도·실데이터 관제 보강

- 모델 없는 응답의 `wait_time=null`을 관광객 화면이 `0분`으로 바꾸던 경로를 제거했다. 대기 보드,
  대안 음성/카드, 저장 목록과 상세 타임라인, 추천 비교 설명은 검증 대기가 없으면 `수집 중`으로
  표시하거나 대기 단계를 생략한다. 정성 혼잡 제보로 처리시간을 곱해 대기를 만들던 코드와 저장
  화면의 가상 총 100석/잔여석도 제거했다.
- 첫 방문의 익명 로그인과 추천 요청이 하나의 Promise를 공유해 고정 mock 사용자로 빠지는 경합을
  없앴다. 온보딩 완료는 로컬 여행 조건을 저장한 즉시 `/main`으로 이동하고 원격 선호 저장은 뒤에서
  처리한다.
- 메인은 24시간 시설 캐시가 있으면 즉시 지도와 추천을 그리고, 없더라도 취향·추정 도보·혜택의
  degraded 순위를 네트워크 전에 먼저 표시한다. `/infrastructures` 첫 응답은 2.5초 안에 없으면
  Supabase 읽기로 전환하며, 서버 결과가 오면 근거 포함 순위로 교체한다.
- 추천 응답의 선택적 LLM 문체 다듬기를 핵심 경로에서 제외해 최대 1.5초 지연을 없앴다. 서버/화면
  문구는 근거 없는 혼잡·대기 주장을 하지 않도록 4개 언어에서 정리했고, 비한국어 화면에 한국어
  추천 사유가 새는 경로도 구조화 값 기반 로컬 문장으로 바꿨다. 검증 모델이 없으면 지도 예측
  슬라이더도 노출하지 않는다.
- 관리자 `GET /api/v1/admin/area-demand-reliability`와 패널을 추가했다. 최근 24시간 10분 버킷의
  수신/누락률·최장 공백·최신성·부모/주차장 상세 일치와 실제 주차면 원본을 보여주며, 장소 내부
  혼잡이 아닌 경주 ITS 주차 수요임을 명시한다.
- SPOT 가중치 `0.4/0.4/0.2`는 변경하지 않았고, 기존 합성 로컬 모델은 계속 운영에서 제외한다.
- 검증: API pytest **725 passed** 및 ruff, web lint 0 errors(기존 warnings 184), typecheck/unit
  (4개 언어 658키 패리티), 정적 build 32 pages, journey-loop Playwright **8/8**(단일 워커),
  스키마 34개 재생성·parity와 `git diff --check` 통과.
...
- 2026-08-20 21:27 KST 운영 스모크는 정상이다. Vercel `https://nextspot-nu.vercel.app`
  ...
  `/api/v1/area-demand/status?lat=35.8400&lng=129.2100` `HTTP 200`(0.330s)을 확인했다.
  지역 수요 응답은 `gyeongju_its` 시설 4곳·실시간 3곳·반경 2km 2곳,
  총 316면·가용 209면(관측 21:28:27 KST)이었다. CI run `32367320418`도 4개 job이 모두 성공했다.
- 경주 ITS가 제공한 실시간 전체/잔여 주차면만 저장한다. 2026-08-24부터는 10분 버킷이며,
  전환 전 15분 원본은 `bucket_minutes=15`로 보존한다.
  `area_demand_snapshots`는 시점별 집계, `area_demand_snapshot_lots`는 주차장별 원본
  관측이다. `record_area_demand_snapshot` RPC가 부모 집계·자식 관측을 한 트랜잭션에서
  교체하며, 같은 소스·버킷 재시도는 멱등이다. 늦게 도착한 오래된 관측은 최신 행을
  되돌리지 않는다.
- 관리자 전용 `POST /api/v1/area-demand/snapshots/collect`와 매시 `3,13,23,33,43,53분` GitHub Actions
  workflow를 추가했다. 수집기는 실측 주차면이 없거나 저장이 실패하면 가짜 행을
  남기지 않고 실패한다.
- 현재 실시간 주차 관측은 관측 시각에서 **30분 이내 도착**에만 적용한다. 30분을
...
- 사용자가 운영 Supabase에 migration을 적용했고, `AREA_DEMAND_COLLECTION_ENABLED=true`를
  ...
  같은 버킷에서 연속 재수집해도 같은 snapshot ID, 부모 1행, 자식 3행을 유지해 운영 DB
  멱등성까지 확인했다. 10분 전환 migration 적용 전까지 운영 스케줄은 기존 15분 상태다.
...
`20260719120000_recommendation_snapshot` · `20260721120000_localdata_sources` 적용 완료.
PostgREST 스키마 캐시도 갱신돼 백엔드가 새 컬럼/테이블을 정상 인식한다(실조회로 확인).
이로써 8개 마이그레이션 전부 적용 상태다.

해소된 퇴화 2건: 추천 설명이 `"추천 장소"` 자리표시자 대신 실제 시설명을 쓰고,
시설 출처가 `"tourapi"` 고정 폴백에서 벗어난다.

### 🔴 실측 — 검증 모델까지의 경로가 사실상 막혀 있다

service_role 로 원격 실데이터를 세어 본 결과(2026-08-21):

| 항목 | 값 |
|---|---|
| congestion_logs 전체 | 2,705 |
| ├ `user_report` | 1,353 |
| └ `seed`(합성, 공개 조회·학습에서 제외) | 1,352 |
| evidence_tier `single_report` | 1,353 |
| **evidence_tier `corroborated`** | **0** |
| **evidence_tier `verified`** | **0** |
| **→ 학습 가능 관측** | **0** / 승격 문턱 300 |
| recommendation_outcomes | 0 |
| model_registry | 0행 (active 0) |
| facility_source_refs | 0 |
| facilities / users / recommendations | 1,619 / 431 / 119 |

**즉 추천 엔진은 지금 `degraded_rules` 로 동작한다** — 혼잡·대기 항이 SPOT 산식에서
통째로 빠지고 취향·실제 이동시간·혜택만 쓴다. 제품의 핵심 차별점인 '도착시점 예측 혼잡'이
프로덕션에서 꺼져 있다.

원인은 버그가 아니다. 코드는 정상이다(`reports.py:170` 이 `reporter_user_id` 를 올바르게 쓴다).
구조가 이렇다:

- `user_report` 1,353건 중 **1,352건이 `reporter_user_id = NULL`** 이다. 그 컬럼은 20260819
  마이그레이션이 추가했고, 기존 행(전부 2026-07-09 백필분)은 채워질 수 없다. 승격 트리거
  `correlate_congestion_report_evidence` 는 `reporter_user_id IS NULL` 이면 즉시 반환하므로
  **이 1,352건은 영원히 승격 불가**다 — 학습 관점에서 죽은 데이터다.
- 실제 유효 제보는 **1건**(2026-08-20 17:05, 폐루프 가동 이후 첫 제보).
- corroborated 승격 조건은 **서로 다른 사용자 2명 이상**이 같은 시설에 **±30분** 안에
  **혼잡도 ±0.05** 로 일치 제보하는 것이다. 현재 트래픽 밀도에서 이 동시발생은 거의 일어나지 않는다.

### 💡 가장 빠른 해소 경로 (설계 변경 없이)

`merchant_report`·`event` 출처는 **단일 소스로 바로 `verified`** 라 상호확인이 필요 없다.
즉 사장님 좌석 방송 1회 = 즉시 학습 가능 관측 1건이다. 사용자 제보를 300쌍 모으는 것보다
...
## -28b. 2026-08-21 — 적용 전 실측 기록 (§6-4 기록 정정)
...
- 음식점·카페는 장소 유형 자체를 실내 자격 근거로 사용하되 `indoor=false` 또는
  `indoor_verified=false`가 있으면 제외하도록 서버·클라이언트·품질 CLI 규칙을 통일했다
  (`6b8a32e`). 문화시설·관광지는 기존대로 명시적 true가 있어야 실내 후보가 된다.
- `/main` 한국어 음성 앱 제어 v1을 추가했다. 카테고리 변경, 실내/우천 모드, 도보 5·10·20분,
  대기 보드 이동만 구조화 명령으로 허용하며 구버전 요청은 `app_context`가 없으면 명령을 활성화하지
  않는다. 후보 0건은 필터·카드를 유지하고 `voice_tool_executed`에는 명령명·상태·정규화 값만
  기록한다. SPOT 산식·가중치·정렬은 변경하지 않았다(`4443404`, `f553657`).
- 음성 명령 Playwright를 추가해 카페 전환, 후보 없는 문화시설 전환의 기존 카드 유지, 대기 보드
  이동을 390×844에서 검증했다(`6bb2bfd`). 최종 평가일 **2026-07-20**, 골든 시나리오 **12개**,
  인증 라이브 하드 실패 **0건**. 로컬 게이트는 API pytest **641 passed**, ruff, 웹 lint **0 errors**
  (기존 warning 187), typecheck/unit/build 32 pages, Playwright **15/15**, 스키마 파리티와
  `git diff --check` 통과.
- 일일 TourAPI showflag 동기화가 공식 임시휴관 중인 시설도 `showflag=1`이면 즉시 재활성화하는 공백을
  수정했다(`07bc7c8`). `features.temporarily_inactive_until` 종료일까지 복구를 보류하고 다음 날부터
  정상 복구한다. 날짜 판정은 CI 서버의 UTC가 아니라 KST 달력일을 사용해 재개관일 오전 9시 지연을
  방지한다(`947a7d5`). 별도 승인 후 중앙도서관에 종료일 `2026-12-28`과 사유 표식을 원격 병합했으며
  적용 감사는 `scratch/library_temporary_closure_guard_apply.json`에만 저장했다. 반영 후 인증 라이브
  12개 시나리오는 하드 실패/경고 **0건**, 활성 장소 **103개**를 유지한다.
- culture 실내 근거 상태를 `미확인`과 `검증된 비실내`로 분리해 라이브 보고서에 기록한다
  (`8189c0d`). 공식 경주시 자료에 따라 교촌마을·황리단길 공예공방거리·최부자댁은 집합 전체가
  우천 대피 가능한 단일 실내 시설이 아니므로 별도 승인 후 원격 `features.indoor_verified=false`를
  병합 반영했다. 적용 감사는 `scratch/non_indoor_evidence_apply.json`에만 저장했다. 최종 인증 라이브
  평가는 활성 장소 **103개**, 12개 시나리오, 하드 실패 **0건**, 경고 **0건**, 실내 근거 미확인
  **0건**, 검증 비실내 **3건**이다.
...
> 자율 개선 세션 로그·재개 규칙: [`AUTONOMOUS_SESSION.md`](./AUTONOMOUS_SESSION.md) · 전략: [`CONTEST_STRATEGY.md`](../contest/CONTEST_STRATEGY.md)
