...
## 2026-08-21 갱신 — 실측 기반 개선 후보 (아래 7월 감사분보다 우선)

> 아래 §0~§4 는 2026-07-07 감사 스냅샷이다. 이 절은 **원격 실데이터를 직접 세어** 도출한
> 현재 시점 우선순위이며, 7월분과 충돌하면 이 절이 우선한다. 근거 수치는
> `docs/HANDOVER.md` §-28.

### C1 (P0) — 검증 모델까지의 경로를 실제로 뚫기

**현상**: 학습 가능 관측 **0건** / 승격 문턱 300. `model_registry` 0행. 추천 엔진이
`degraded_rules` 로 동작 중이라 **'도착시점 예측 혼잡'이 프로덕션에서 꺼져 있다.**

**진단**: 버그가 아니라 구조다. `user_report` 1,353건 중 1,352건은 `reporter_user_id` 가
NULL 인 백필분이라 승격 트리거가 즉시 반환한다 — 영원히 학습에 못 쓴다. 유효 제보는 1건뿐이고,
corroborated 승격은 서로 다른 사용자 2명이 ±30분·±0.05 로 일치해야 해서 현재 밀도에서는
사실상 발생하지 않는다.

**권고 — 설계를 바꾸지 말고 빠른 경로를 쓸 것.** `merchant_report`·`event` 출처는 단일 소스로
바로 `verified` 다(상호확인 불필요). 사장님 좌석 방송 1회 = 즉시 학습 가능 관측 1건.
현재 이 경로가 **0건**으로 한 번도 안 쓰였다. 시연 매장 몇 곳 + 관리자 확인으로 300건을
채우는 것이 사용자 제보 300쌍을 기다리는 것보다 압도적으로 빠르다.

**하지 말 것**: 승격 문턱(300 / MAE 0.15 / 베이스라인 20%)이나 evidence_tier 규칙을 낮춰서
맞추는 것. 그 게이트가 이 프로젝트에서 가장 잘 설계된 부분이고, 낮추면 '합성 데이터로 만든
모델이 실측인 척'하는 바로 그 문제로 되돌아간다.

### C2 (P1) — 무해 폴백이 조용히 삼키는 장애를 드러내기

**현상**: 마이그레이션 2건이 원격에 빠져 있었는데 **아무도 몰랐다.** 코드에 폴백이 잘 만들어져
있어서 앱이 죽지 않았고, 대신 추천 설명이 `"추천 장소"` 자리표시자로, 시설 출처가 `"tourapi"`
고정으로 조용히 퇴화한 채 돌았다.

**진단**: 무해 폴백은 이 저장소의 핵심 강점이다. 동시에 **폴백이 좋을수록 갭이 오래 숨는다.**
`infrastructures.py:177` 은 `facility_source_refs_unavailable` 경고를 계속 찍고 있었지만
아무도 보지 않는 로그였다.

**권고**: '설계된 폴백'과 '설정 누락으로 인한 폴백'을 구분해 후자만 눈에 띄게 한다.
관리자 대시보드에 폴백 상태 배지(스키마 미적용·키 미설정·LLM 비활성)를 두거나,
`/health` 를 확장해 degraded 항목을 나열하는 방식. **자동 복구는 하지 말 것** — 관측만
드러내고 조치는 사람이 판단한다.

### C3 (P1) — `/saved` 라이브 혼잡 재조회 미배선

`applyLive`(35줄)가 완성돼 있고 `/infrastructures` 도 받아오는데 렌더가 호출하지 않는다.
결과적으로 저장 페이지가 **저장 당시의 옛 혼잡도를 현재값처럼** 보여준다 — 이 저장소의
'정직한 데이터 표시' 원칙에 정면으로 어긋난다. 배선하려면 '저장 당시 기준' 라벨의 4로케일
문구가 필요하다(현재 i18n 키 없음) → 제품 판단 필요.

### C4 (P2) — 남은 lint warning 132건 중 70건이 `react-hooks/*`

`set-state-in-effect` 50 · `exhaustive-deps` 10 · `purity` 9 · `refs` 6 · `immutability` 5.
전부 렌더 타이밍이 바뀔 수 있어 기계적으로 밀면 안 된다. 나머지 60건은 도메인 데이터 형태
`any` 라 타입 정의가 선행돼야 한다.

### C5 (P2) — PR #4 revert 미복구

`feature/ui-editorial-pass` 의 UI 개편 219줄 + `onboarding-usability.spec.ts` 가 main 에서
빠져 있다. 머지 10분 뒤 revert 라 **사유 파악이 선행**이다. 작업물은 원격 브랜치에 남아 있다.
