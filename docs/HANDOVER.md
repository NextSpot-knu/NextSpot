...
- **main = 프로덕션.** main push가 Vercel(web)·Render(api)를 자동 배포한다. 마지막 반영은 2026-09-26 — 운영 긴급 수정 3건 + 메모리 근원 차단(`0ae42b8`..`b3eefef`, 아래 2026-09-26). 그 전 2026-09-25 API OOM 대응(`0408bd7`..`0ce394d`). 그 전 2026-09-22 `ec127ee`, 09-21 —
...
- 다음·미결: 관광객 CPU(대기 보드 명소·문화 20~25초, 코스 35~42초 — 프로파일링·동일 출력 최적화 워크플로 진행 중) ·
  재시도 전송의 풀 전체 닫기(중복 POST·Linux 최대 120초 멈춤 — 대안 실측 중) · /account/me 401 시 세션 갱신 후 1회 재시도(운영 401 0건).
