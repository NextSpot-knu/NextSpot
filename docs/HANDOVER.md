...
- **main = 프로덕션.** main push가 Vercel(web)·Render(api)를 자동 배포한다. 마지막 반영은 2026-09-20 —
  `740a8ce`(소개 첫 화면을 사용자 효익·3단계 체험으로 단순화, SPOT 계산식·데이터 출처는 선택형 심사 근거로 보존).
  CI 성공 뒤 Vercel `/guide` HTTP 200과 새 제목·3단계·무가입·계산 근거 문구를 응답에서 확인했다.
...
- 도구·브랜치: Codex / `feature/judge-guide` → main (`740a8ce`).
...
- 다음·미결: 없음. CI `35510338898` 성공, Vercel 배포 응답에서 새 첫 화면 문구 확인.
