...
실측 혼잡 로그 2,704행 기준 시계열 홀드아웃(최근 20% 구간 분리) 평가에서 **MAE 0.0802** — 시간대 평균으로
찍는 단순 기준선(MAE 0.2159) 대비 **63% 개선**, 학습 R²=0.73이다(2026-07-14 `train.py --evaluate` 실행,
지표는 model.pkl에 내장되어 관리자 화면 정확도 배지와 `GET /predict/model-info`로 그대로 노출된다).
핵심은 이 수치가 "주장"이 아니라 그 자리에서 같은 명령으로 재현 가능한 검증 절차의 산출물이라는 점이며,
예측 서비스는 CI 게이트(`pytest apps/api` 118 passed)로 회귀 안정성도 별도로 보장받는다.
[뒷받침 근거: train.py --evaluate(홀드아웃 MAE vs 기준선), GET /predict/model-info, docs/HANDOVER.md §4 CI 게이트]
...
(`INCENTIVE_COUPON_SHARE=0.5`)을 차지해, 추천되는 대안이 곧 제휴 소상공인이 되도록 구조화했다.
